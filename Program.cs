...
) =>
{
    ...
    {
        Console.Error.WriteLine("오류: 분석할 대상 저장소를 입력해 주세요.");
        return;
    }

    if (repos.Length > 1)
    {
        Console.Error.WriteLine("알림: 여러 저장소 합산 기능은 아직 구현되지 않았습니다. 첫 번째 저장소만 분석을 진행합니다.\n");
    }

    // 첫 번째 저장소만 추출하여 기존 로직 그대로 진행
    var targetRepo = repos[0];
    var parts = targetRepo.Split('/');
    if (parts.Length != 2)
    {
        Console.Error.WriteLine($"오류: '{targetRepo}'는 'owner/repo' 형식이 아닙니다.");
        return;
    }
    ...
    try
    {
        ...
        Console.Error.WriteLine($"{targetRepo} 기여자 데이터 수집 및 분석 중...");

        if (!Directory.Exists(output)) Directory.CreateDirectory(output);
        string cachePath = Path.Combine(output, "cache.json");
        var cache = CacheManager.LoadCache(cachePath, targetRepo);
        ...
        if (format.ToLower() == "txt")
        {
            ...
            string txtContent = BuildTextReport(targetRepo, reportData);
